...
enum TagVocabulary {
    ...
    /// The App Group's defaults, not the app's own: the share extension
    /// reads the vocabulary too, to offer a shared spot's tags and let the
    /// model pick from them (§8.1). Every `@AppStorage` of `storageKey`
    /// passes this as its `store`.
    static let defaults = UserDefaults(suiteName: SharedStore.appGroupID) ?? .standard

    /// The vocabulary lived in `UserDefaults.standard` until the extension
    /// needed it. Copy it across once; the old value is left where it was.
