...
| a social post (`instagram.com`, `threads.net`, `x.com`, …) | `OGMetadataFetcher` | `title`, `imageURL`, `urlString`, `notes` (`og:description` — the caption, with the network's byline stripped); no `venue`, the site name isn't one. With `SpotExtractor`, the caption path then replaces the title with the place's name and the caption with a summary |
